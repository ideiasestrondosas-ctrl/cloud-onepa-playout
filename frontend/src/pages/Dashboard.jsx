...
export default function Dashboard() {
  ...
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      {/* ===== PLAYOUT HEALTH PANEL ===== */}
      {(() => {
        // Health dimension scores (each 0–1)
        const engineOk  = isPlaying ? 1 : 0;
        const schedOk   = debugReport?.has_active_schedule ? 1 : (debugReport ? 0 : 0.5);
        const playlistOk= debugReport?.has_playlist ? 1 : (debugReport ? 0 : 0.5);
        const mediaOk   = debugReport ? Math.min(1, (debugReport.media_files_count || 0) / 5) : 0.5;
        const protoOk   = status.active_streams?.some(s => s.status === 'active') ? 1 : (isPlaying ? 0.4 : 0);
        const errorPenalty = status.last_error ? 0.15 : 0;

        const rawScore = ((engineOk + schedOk + playlistOk + mediaOk + protoOk) / 5) - errorPenalty;
        const healthScore = Math.round(Math.max(0, Math.min(1, rawScore)) * 100);

        const scoreColor = healthScore >= 80 ? '#4caf50' : healthScore >= 50 ? '#ff9800' : '#f44336';
        const scoreLabel = healthScore >= 80
          ? t('dashboard.health.score_good')
          : healthScore >= 50
          ? t('dashboard.health.score_warn')
          : t('dashboard.health.score_poor');

        // Circle SVG constants
        const R = 32;
        const CIRC = 2 * Math.PI * R;
        const dash = (CIRC * healthScore) / 100;

        const checks = [
          {
            key: 'engine',
            label: t('dashboard.health.check_engine'),
            ok: engineOk >= 1,
            na: engineOk === 0.5,
            detail: isPlaying ? t('dashboard.health.running') : t('dashboard.health.stopped'),
          },
          {
            key: 'schedule',
            label: t('dashboard.health.check_schedule'),
            ok: schedOk >= 1,
            na: !debugReport,
            detail: debugReport?.has_active_schedule
              ? (debugReport.active_schedule_name || t('dashboard.health.active'))
              : t('dashboard.health.no_schedule'),
          },
          {
            key: 'playlist',
            label: t('dashboard.health.check_playlist'),
            ok: playlistOk >= 1,
            na: !debugReport,
            detail: debugReport?.has_playlist
              ? (debugReport.playlist_name || t('dashboard.health.loaded'))
              : t('dashboard.health.no_playlist'),
          },
          {
            key: 'media',
            label: t('dashboard.health.check_media'),
            ok: mediaOk >= 1,
            na: !debugReport,
            detail: debugReport
              ? t('dashboard.health.clips_count', { count: debugReport.media_files_count || 0 })
              : t('dashboard.health.loading'),
          },
          {
            key: 'protocols',
            label: t('dashboard.health.check_protocols'),
            ok: protoOk >= 1,
            na: !isPlaying,
            detail: isPlaying
              ? (status.active_streams?.filter(s => s.status === 'active').length || 0) + ' ' + t('dashboard.health.active_streams')
              : t('dashboard.health.engine_offline'),
          },
        ];
          ...
          <Box sx={{ mt: 2, position: 'relative', zIndex: 1 }}>
            <Paper className="glass-panel" sx={{
              ...
              borderColor: isPlaying
                ? (healthScore >= 80 ? 'rgba(76,175,80,0.25)' : healthScore >= 50 ? 'rgba(255,152,0,0.25)' : 'rgba(244,67,54,0.25)')
                : 'rgba(255,255,255,0.06)',
              boxShadow: isPlaying && healthScore >= 80
                ? '0 0 20px rgba(76,175,80,0.06)'
                : isPlaying && healthScore < 50
                ? '0 0 20px rgba(244,67,54,0.06)'
                : 'none',
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
                ...
                {debugReport?._ts && (
                  <Typography variant="caption" sx={{ color: 'text.disabled', fontFamily: 'monospace', fontSize: '0.6rem' }}>
                    {t('dashboard.health.updated_at', { time: debugReport._ts.toLocaleTimeString() })}
                  </Typography>
                )}
              </Box>
              ...
              <Grid container spacing={2} alignItems="center">
                ...
                <Grid item xs={12} sm="auto">
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5, minWidth: 96 }}>
                    <Box sx={{ position: 'relative', width: 84, height: 84 }}>
                      ...
                      <Box sx={{
                        ...
                        borderRadius: '50%',
                        border: '2px solid',
                        borderColor: isPlaying ? `${scoreColor}22` : 'rgba(255,255,255,0.04)',
                        animation: isPlaying ? 'onair-blink 3s ease-in-out infinite' : 'none',
                      }} />
                      ...
                      <Box sx={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
                        <Typography sx={{ fontWeight: 900, fontSize: '1.35rem', lineHeight: 1, color: scoreColor, fontFamily: '"Orbitron", sans-serif', transition: 'color 0.5s' }}>
                          {healthScore}
                        </Typography>
                        <Typography sx={{ fontSize: '0.55rem', color: 'text.disabled', letterSpacing: 0.5, mt: 0.2 }}>/ 100</Typography>
                      </Box>
                    </Box>
                    <Typography sx={{ fontSize: '0.6rem', fontWeight: 800, color: scoreColor, letterSpacing: 1, textTransform: 'uppercase', transition: 'color 0.5s' }}>
                      {scoreLabel}
                    </Typography>
                  </Box>
                </Grid>

                <Divider orientation="vertical" flexItem sx={{ opacity: 0.08, mx: 0.5, display: { xs: 'none', sm: 'block' } }} />

                {/* System Check Cards */}
                <Grid item xs={12} sm>
                  <Grid container spacing={1}>
                    {checks.map(check => {
                      const dotColor = check.na ? '#444' : check.ok ? '#4caf50' : '#f44336';
                      const bgColor  = check.na ? 'rgba(255,255,255,0.02)' : check.ok ? 'rgba(76,175,80,0.06)' : 'rgba(244,67,54,0.06)';
                      const borderColor = check.na ? 'rgba(255,255,255,0.05)' : check.ok ? 'rgba(76,175,80,0.2)' : 'rgba(244,67,54,0.2)';
                      return (
                        <Grid item xs={12} sm={6} md key={check.key}>
                          <Box sx={{
                            p: 1, borderRadius: 2,
                            bgcolor: bgColor,
                            ...
                            <Box sx={{
                              width: 8, height: 8, borderRadius: '50%', flexShrink: 0, mt: '3px',
                              bgcolor: dotColor,
                              boxShadow: check.ok && !check.na ? `0 0 6px ${dotColor}` : 'none',
                            }} />
                            <Box sx={{ overflow: 'hidden', minWidth: 0 }}>
                              <Typography sx={{ fontSize: '0.6rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: 0.8, color: 'text.secondary', lineHeight: 1 }}>
                                {check.label}
                              </Typography>
                              <Typography noWrap sx={{ fontSize: '0.68rem', fontWeight: 600, color: check.na ? 'text.disabled' : check.ok ? '#4caf50' : '#f44336', mt: 0.3, transition: 'color 0.4s' }}>
                                {check.detail}
                              </Typography>
                            </Box>
                          </Box>
                ...
                {status.last_error && (
                  <Grid item xs={12}>
                    <Box sx={{
                      ...
                      <Typography noWrap sx={{ fontSize: '0.68rem', fontFamily: 'monospace', color: 'error.main', fontWeight: 600 }}>
                        {status.last_error}
                      </Typography>
                    </Box>
                  </Grid>
