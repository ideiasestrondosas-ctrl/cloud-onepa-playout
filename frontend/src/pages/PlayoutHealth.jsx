...
export default function PlayoutHealth() {
  ...
  const wsChipLabel = wsStatus === 'connected' ? t('websocket.connected') : wsStatus === 'connecting' ? t('websocket.connecting') : t('websocket.offline');
      ...
      {pageTab === 1 && (
        <>
          ...
          <Grid container spacing={2} mb={3}>
            <Grid item xs={6} md={3}>
              <KPICard label={t('analytics.kpis.wsStatus')} value={wsStatus === 'connected' ? 'LIVE' : 'OFFLINE'} icon={TimelineIcon} color={wsStatus === 'connected' ? theme.palette.success.main : theme.palette.error.main} />
            </Grid>
            <Grid item xs={6} md={3}>
              <KPICard label={t('analytics.kpis.dataPoints')} value={streamHealth.length} icon={MonitorIcon} color={theme.palette.primary.main} />
            </Grid>
            <Grid item xs={6} md={3}>
              <KPICard label={t('analytics.kpis.clips7Days')} value={clipsPerDay.reduce((s, d) => s + (d.count || 0), 0)} icon={ReceiptLongIcon} color={theme.palette.info.main} />
            </Grid>
            <Grid item xs={6} md={3}>
              <KPICard label={t('analytics.kpis.errorRate')} value="0%" icon={ReportProblemIcon} color={theme.palette.warning.main} />
            </Grid>
          </Grid>
...
<Typography variant="subtitle2" sx={{ mb: 2, textTransform: 'uppercase', letterSpacing: 1, fontSize: '0.72rem', fontWeight: 800 }}>
                     {t('analytics.charts.liveBitrate')}
                   </Typography>
...
{analyticsLoading ? (
                     ...
                     <Box sx={{ py: 6, textAlign: 'center' }}>
                       <Typography variant="body2" color="text.secondary">{t('analytics.noAsRunData')}</Typography>
                     </Box>
                  ) : (
                    <ResponsiveContainer width="100%" height={220}>
