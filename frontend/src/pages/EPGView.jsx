...
export default function EPGView() {
    ...
    return (
        <Box sx={{ height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column', position: 'relative' }}>
            {/* Background Glow — reduced to a subtle dot to avoid heavy GPU blur */}
            <Box sx={{
                position: 'absolute',
                top: '20%',
                right: '10%',
            ...
            <Box
                ...
                onMouseDown={(e) => {
                    // Throttled drag to scroll using requestAnimationFrame — prevents layout thrashing
                    ...
                    const onMouseMove = (ev) => {
                        lastPageX = ev.pageX;
                        if (rafId) return; // throttle: wait for next frame
                        rafId = requestAnimationFrame(() => {
                            const x = lastPageX - ele.offsetLeft;
