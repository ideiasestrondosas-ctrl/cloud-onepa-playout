    ...
    const [settings, setSettings] = useState(null);
     ...
     const fetchData = useCallback(async () => {
         try {
             ...
             // Ensure the default channel is included (special ID that cannot be deleted)
             const defaultChannelId = '00000000-0000-0000-0000-000000000001';
             if (!list.find(c => c.id === defaultChannelId)) {
                 try {
                     const defaultChannelRes = await channelsAPI.get(defaultChannelId);
                     list = [...list, defaultChannelRes.data];
                 } catch (err) {
                     // If we can't fetch the default channel, log and proceed without it
                     console.warn('Could not fetch default channel', err);
                 }
             }
                            ...
                            >
                                ...
                                <CardContent>
                                    ...
                                    <Box sx={{ mb: 2 }}>
                                        ...
                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <Typography variant="caption" color="text.secondary">
                                                {t('multiChannel.bitrate', 'Bitrate')}
                                            </Typography>
                                            <Typography variant="caption">
