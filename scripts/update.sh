...
echo -e "  📡 RTMP (Ext):    ${CYAN}rtmp://$(hostname -I 2>/dev/null | awk '{print $1}' || echo 'localhost'):1935/live/stream${NC} (Apenas ativo em ON AIR)"
echo -e "  🔗 SRT (Ext):     ${CYAN}srt://$(hostname -I 2>/dev/null | awk '{print $1}' || echo 'localhost'):8890?streamid=read:live/stream_srt${NC}"
echo ""
